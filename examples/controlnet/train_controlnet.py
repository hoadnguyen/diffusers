...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="init_image",
        help="The column of the dataset containing the init image.",
    )
    parser.add_argument(
        "--mask_image_column",
        type=str,
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.init_image_column is None:
        init_image_column = column_names[3]
        logger.info(f"init image column defaulting to {init_image_column}")
    else:
        init_image_column = args.init_image_column
        if init_image_column not in column_names:
            raise ValueError(
                f"`--init_image_column` value '{args.init_image_column}' not found in dataset columns. Dataset columns are: {', '.join(column_names)}"
            )

    if args.mask_image_column is None:
        mask_image_column = column_names[4]
    ...
    def preprocess_train(examples):
        ...
        init_images = [image.convert("RGB") for image in examples[init_image_column]]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # Convert init images to latent space
                init_latents = vae.encode(
                    batch["init_pixel_values"].to(dtype=weight_dtype)
                ).latent_dist.sample()
                ...
                # Concat the init latents and mask to the noisy latents for inpainting
                latent_model_input = torch.cat(
                    [noisy_latents, mask, init_latents], dim=1
                )
